...
Stacks are LIFO, which means we will typically be impacting the top element with stack operations. A linked list is efficient in this situaton because we keep track of the top/head node, allowing insertion an deletion with our push and pop operations without needing to shift elements. And since linked list do no require contiguous memory like arrays do we have more flexibility with our memory while maintaining a constant time complexity.
